...
import { renderTipoCostoOptions, renderEnlacePanel, renderCeldaCantidadInsumo, renderHelp, renderBuscador, renderTarjetaMini, renderExploradorInsumos } from "../core/components.js";
import { subirImagenReferencia } from "../core/drive.js";
import { calcCostoUnitarioRef, esInsumoServicio, cantidadEfectivaInsumo } from "../core/calc.js";
import { TIPOS_ENLAZABLES } from "../core/constants.js";
...
function renderPlantillaCard(p) {
  ...
  html += '<div class="ins-table"><div class="ins-row head" style="grid-template-columns:' + INS_COLS + ';"><span>Insumo</span><span>Unidad</span><span class="ins-th-num">Costo</span><span>Tipo de costo</span><span>Enlace</span><span class="ins-th-num">Cant./mult.</span><span></span></div>';
  insumos.forEach(function (i) {
    ...
    var attrsIdent = ' data-pla="' + p.id + '" data-ins="' + i.id + '"';
    var attrsIns = ' data-action-change="set-pla-ins-campo"' + attrsIdent;
    html += '<div class="ins-row" style="grid-template-columns:' + INS_COLS + ';" data-ins-row data-pla="' + p.id + '" data-ins="' + i.id + '">' +
      ...
      '<button class="ins-remove-btn" data-action="remove-pla-insumo" data-pla="' + p.id + '" data-ins="' + i.id + '" title="Quitar" aria-label="Quitar">✕</button>' +
      "</div>";
  });
...
export var actions = {
  ...
  "toggle-pla-ins-enlace-categoria": function (el) {
    var id = el.getAttribute("data-pla"), insId = el.getAttribute("data-ins"), catId = el.getAttribute("data-cat");
    mapPla(id, function (p) {
      var insumos = (p.insumos || []).map(function (i) {
        if (i.id !== insId) return i;
  ...
  "toggle-pla-ins-enlace-insumo": function (el) {
    var id = el.getAttribute("data-pla"), insId = el.getAttribute("data-ins"), nombreClave = el.getAttribute("data-nombre");
    mapPla(id, function (p) {
      var insumos = (p.insumos || []).map(function (i) {
        if (i.id !== insId) return i;
        var enlace = i.enlace || { categorias: [], insumos: [] };
